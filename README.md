...
### 数据同步与可复现性（导入手动收集的 PMID）
如果您在使用网页端进行审阅时，使用了右上角的 **"📥 导入 PMID 文件"** 补充了额外的 PubMed 核心文献，系统会自动将这些 PMID 写入根目录的 `manual_imported_pmids.txt` 中。
**在其他人（或者您在新的服务器）重新构建仓库和复现最初完整的检索流程时**，运行 `python main.py` 会自动检测该文本文件，同步把手动指定的 PMID 文章内容一起爬取并合并到您的初始文献中。这保证了所有依靠人工引入的分散重要文献**不会因为重新拉取代码或者重新建库而丢失**。
