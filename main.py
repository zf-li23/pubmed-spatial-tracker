...
def main():
    ...
    # 1.5 抓取手动导入的文献（从 SQLite 表读取，保障可复现）
    try:
        with engine.connect() as con:
            result = con.execute(text("SELECT pmid FROM manual_imported_pmids"))
            manual_ids = [str(row[0]).strip() for row in result.fetchall() if str(row[0]).strip().isdigit()]
        ...
        print(f"读取数据库 manual_imported_pmids 失败: {e}")
